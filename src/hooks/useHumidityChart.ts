...
import { HumidityChartData, UseHumidityChartProps } from '@/types/humidityChart';
import { aggregateData } from '@/utils/charts';

export function useHumidityChart({ data, selectedDevices, timeRange }: UseHumidityChartProps): HumidityChartData {


  return useMemo(() => {
    const validSelectedDevices = selectedDevices.filter(d => d !== '');

    const timestamps = [...new Set(data.map(d => d.TMS * 1000))].sort((a, b) => a - b);
